...
export const callShaderSnippet = (
    instance: ShaderSnippetInstance,
    params: Array<string>,
    resultObjs?: Array<string>
) => {
    if (instance.snippet.params.length !== params.length) {
        throw new Error(
            `Missing params in ${instance.name} call, expecting params: ${instance.snippet.params
    ...
    if (resultObjs) {
        if (resultObjs.length !== instance.resultObjs.length) {
            throw new Error(
                `Missing result object in ${instance.name} call, expecting results: ${instance.resultObjs
                    .map((resultObj) => resultObj.name)
                    .join(', ')}`
            );
        }
    } else {
