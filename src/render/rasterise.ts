...
const getLineRange = (line: Line): LineRange => {
    ...
    return {
        pStart,
        pEnd,
        xRange: vec2.fromValues(Math.min(line.p1[0], line.p2[0]), Math.max(line.p1[0], line.p2[0])),
...
export const rasterise = (
    ...
    if (maxPos[1] - minPos[1] < 1.0) {
        callback(minPos[0], maxPos[0], minPos[1], false);
        return;
    }

    let prevY = Math.floor(minPos[1]);

    for (let y = prevY + 1; y <= Math.ceil(maxPos[1]); y++) {
